...
- EMQX管理：Mqtt客户端、监听器、消息主题、消息订阅、插件管理
- 物模型管理：属性（设备状态和监测数据，可读取和设置），功能（执行特定任务，可读取和设置），事件（设备主动上报给云端，设备故障、温度过高）
- 产品管理：产品、产品物模型、产品分类、产品固件等
- 设备管理：设备、设备分组、设备定时、设备控制、设备影子、实时监测、设备日志、监测统计、设备升级、设备定位、设备加密认证等
- 硬件SDK：支持WIFI连接、MQTT连接、物模型响应、实时监测、管理平台配置定时上传监测数据、AES加密、Http和Mqtt获取NTP时间等
