...
public class DeviceUserController extends BaseController
{
     ...
     * 查询设备用户列表
     */
    @PreAuthorize("@ss.hasPermi('iot:deviceUser:list')")
    @GetMapping("/list")
     ...
     * 获取设备用户详细信息
     */
    @PreAuthorize("@ss.hasPermi('iot:deviceUser:query')")
     ...
     * 新增设备用户
     */
    @PreAuthorize("@ss.hasPermi('iot:deviceUser:add')")
     ...
     */
    @ApiOperation("修改设备用户")
    @PreAuthorize("@ss.hasPermi('iot:deviceUser:edit')")
    ...
    @PreAuthorize("@ss.hasPermi('iot:deviceUser:remove')")
